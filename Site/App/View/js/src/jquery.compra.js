...
async function reloadTableProduct() {
  ...
  $.ajax({
    ...
    success: async function (result) {   
        ...
        lista_produtos.push({ id_produto: result.response_data.id, quantidade: $('#quantidade').val(), valor_unit: $('#valor_unitario').val() })

        // Adicionando produto no carrinho de compra
        $('#tableProduto').append(`<tr> 
       <td> ${result.response_data.descricao} </td> 
       <td> ${Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format($('#valor_unitario').val().toString())} </td>
...
function updateParcelasValue(qnt_parcelas) {
    let valor_liquido = (valor_total / qnt_parcelas).toFixed(2)
  $('#valor_parcela').val(  Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(valor_liquido))
